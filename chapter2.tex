...
Let $M_0, M_1, \ldots$ be at most countable sets. If $M_n = \emptyset$ for all $n \in \nat$, then $\bunion_{n \in \nat} M_n = \emptyset$, which is clearly at most countable. Otherwise, let $\beta_n: M_n \to \nat$ be an injective map given by \reftitle{Lemma~1.1(c)} for all $n \in \nat$ such that $M_n \neq \emptyset$.\par
...
%%
\item $k > 0$: Let $t = \xi^\prime \zeta^\prime$. Then the first symbol of $\zeta^\prime$ must lie somewhere in $t_j$ for some $1 \leq j \leq n$. By induction hypothesis, there are strings $\xi^{\prime \prime}$ and $\eta^{\prime \prime}$ $\in \mathcal{A}_S^*$ and $t^{\prime \prime} \in T^S$ such that $t_j = \xi^{\prime \prime} t^{\prime \prime} \eta^{\prime \prime}$. Specifically, $\xi^{\prime \prime}$ is a postfix of $\xi^\prime$ and $\eta^{\prime \prime}$ is a prefix of $\eta^\prime$ and, at the same time, $t^{\prime \prime} = t^\prime$.\nolinebreak\hfill$\talloblong$
\end{enumerate}

\begin{remark}
In fact, every term can be seen as a ``flattened'' formation tree in which variables or constants occur as leaf nodes whereas function symbols (if any) occur as internal nodes. In this view, the word $t_1 \etl t_n$ is a flattened forest and at each place it is the root of exactly one tree which is a subtree of some $t_i$.
\end{remark}
\end{enumerate}
%End of II.4.9-----------------------------------------------------------------------------------------------
%End of II.4-------------------------------------------------------------------------------------------------
